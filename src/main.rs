...
async fn main() {
    ...
    let pool = PgPool::connect(database_url)
        .await
        .unwrap_or_else(|_| panic!("failed to connect database, url is [{}]", database_url));
...
// テスト対象を分離するために、create_appを切り出している
...
mod test {
    ...
    async fn should_created_todo() {
        ...
        // oneshot: リクエストを送信し、Routerによって処理されたレスポンスを返す
        .oneshot(req)
        .await
        .unwrap();
    ...
    async fn should_update_todo() {
        ...
        todo_repository
            .create(CreateTodo::new(
                "before_should_update_todo".to_string(),
                label_ids,
            ))
            .await
            .expect("failed to create todo");
    ...
    async fn should_all_label_readed() {
        ...
        let labels: Vec<Label> = serde_json::from_str(&body)
            .unwrap_or_else(|_| panic!("cannot convert Label list instance. body: {}", body));
