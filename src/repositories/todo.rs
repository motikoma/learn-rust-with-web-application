...
impl TodoRepository for TodoRepositoryForDb {
    ...
    async fn update(&self, id: i32, payload: UpdateTodo) -> anyhow::Result<TodoEntity> {
        ...
        // ラベルは複数あるので一度全部削除してから再登録する
        if let Some(labels) = payload.labels {
            sqlx::query(
                r#"delete from todo_labels
                where todo_id = $1
                "#,
            )
            .bind(id)
...
struct TodoWithLabelFromRow {
    ...
    text: String,
    completed: bool,
    label_id: Option<i32>, // labelが紐づいていない場合はnull
...
mod test {
    ...
    async fn crud_scenario() {
        ...
        .fetch_optional(&pool)
        .await
        .expect("Failed tod prepare label data.");
        ...
        assert_eq!(created.labels, vec![label_1]);
...
pub mod test_utils {
    ...
    pub struct TodoRepositoryForMemory {
        // Arc<RwLock>はスレッドセーフなHashMapを実現するためのラッパー
        // 不変参照の時は複数スレッドで共有できるが、可変参照の時は一つのスレッドしか持てない
    ...
    impl TodoRepositoryForMemory {
        ...
        pub fn write_store_ref(&self) -> RwLockWriteGuard<TodoDatas> {
            self.store.write().unwrap()
        }

        pub fn read_store_ref(&self) -> RwLockReadGuard<TodoDatas> {
    ...
    impl TodoRepository for TodoRepositoryForMemory {
        async fn create(&self, payload: CreateTodo) -> anyhow::Result<TodoEntity> {
            ...
            let todo = TodoEntity::new(id, payload.text, labels);
